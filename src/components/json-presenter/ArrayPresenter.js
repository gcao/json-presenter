    ...
    render() {
        ...
        if (data.length > 0) {
            if (R.isArrayLike(data[0])) {
                return (<ArrayOfArraysPresenter data={data} path={path} config={config}/>);
            } else if (R.is(Object, data[0])) {
                return (<ArrayOfObjectsPresenter data={data} path={path} config={config}/>);
            }
        } else {
            return (
                <table
                ...
                >
                    {
                        data.map((item, i) =>
                            ...
                            >
                                <td>
                                    <JsonPresenter data={item} path={path.append(i)} config={config}/>
                                </td>
                            </tr>
                        )
                    }
